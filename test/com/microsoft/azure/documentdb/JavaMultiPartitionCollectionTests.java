...
public class JavaMultiPartitionCollectionTests extends GatewayTestBase {
    ...
    public JavaMultiPartitionCollectionTests() {
		super();
	}
    ...
    public void testCollectionCreate() throws DocumentClientException {
        ...
        Assert.assertNotNull(
                this.client.getPartitionKeyDefinitionMap().getPartitionKeyDefinition(createdCollection.getSelfLink()));
    ...
    private void testCrossPartitionDocumentQueryIterable(DocumentCollection coll, Document[] documents,
        ...
        // Max Parallelized FeedOptions
        FeedOptions maxFeedOptions = new FeedOptions();
        maxFeedOptions.setEnableCrossPartitionQuery(true);
        maxFeedOptions.setPageSize(-1);
        maxFeedOptions.setMaxBufferedItemCount(Integer.MAX_VALUE);
        maxFeedOptions.setMaxDegreeOfParallelism(Integer.MAX_VALUE);
        ...
        for (int trial = 0; trial < 1; ++trial) {
            for (boolean useFetchNextBlock : Arrays.asList(true, false)) {
                for (FeedOptions feedOptions : Arrays.asList(defaultFeedOptions, serialFeedOptions, maxFeedOptions,
    ...
    public void testPartitionKeyCache() throws DocumentClientException {
        ...
        Assert.assertNotNull(
                this.client.getPartitionKeyDefinitionMap().getPartitionKeyDefinition(createdCollection.getSelfLink()));
        String collectionLink = TestUtils.getDocumentCollectionLink(this.databaseForTest, createdCollection, true);
        ...
        Assert.assertNotNull(
                this.client.getPartitionKeyDefinitionMap().getPartitionKeyDefinition(createdCollection.getSelfLink()));
        collectionLink = TestUtils.getDocumentCollectionLink(this.databaseForTest, createdCollection, true);
        Assert.assertNotNull(this.client.getPartitionKeyDefinitionMap().getPartitionKeyDefinition(collectionLink));
    }
