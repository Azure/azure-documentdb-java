...
final class DocumentProducer implements Iterator<Document> {
    ...
    public DocumentProducer produce() throws DocumentClientException {
        ...
        if (this.parentQueryExecutionContext.shouldExecuteQuery()) {
            response = this.parentQueryExecutionContext.executeQueryRequest(this.request);
        } else {
            response = this.parentQueryExecutionContext.executeReadFeedRequest(this.request);
        }
