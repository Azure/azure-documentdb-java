...
class QuorumReader {
    ...
    private final static int DELAY_BETWEEN_READ_BARRIER_CALLS_IN_MS = 5;
    ...
    private ReadQuorumResult readQuorum(DocumentServiceRequest request, int readQuorum, boolean includePrimary) throws DocumentClientException {
        List<StoreReadResult> responseResult = this.storeReader.readMultipleReplicaImpl(request, includePrimary, readQuorum);
        ...
        // If the replicas are not on the same LSN, we ping the replicas with a head request ( Barrier request ) MAX_NUMBER_OF_READ_BARRIER_RETRIES times to see 
