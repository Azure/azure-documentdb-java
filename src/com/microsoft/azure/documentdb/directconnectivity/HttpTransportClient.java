...
public class HttpTransportClient extends TransportClient {
    ...
    public StoreResponse invokeStore(URI physicalAddress, DocumentServiceRequest request) throws DocumentClientException {
        ...
        return this.processResponse(response, request.getActivityId());
    }

    public StoreResponse processResponse(HttpResponse response, String activityId) throws DocumentClientException {
        if (response == null) {
        ...
        if (response.getStatusLine().getStatusCode() < 300 || response.getStatusLine().getStatusCode() == 304) {
            ...
            ErrorUtils.maybeThrowException(response, false, this.logger);
            return null;
        }
    }
    ...
    public HttpRequestBase prepareHttpMessage(String activityId, URI physicalAddress, OperationType operationType,
        ...
        String resourceUrl = physicalAddress + Utils.trimBeginingAndEndingSlashes(request.getPath());
        switch (operationType) {
            ...
            case Replace:
                HttpPut putRequest = new HttpPut(resourceUrl);
            ...
            case Delete:
                HttpDelete deleteRequest = new HttpDelete(resourceUrl);
            ...
            case Query:
            case SqlQuery:
                HttpPost queryPostRequest = new HttpPost(resourceUrl);
